...
/*
You shold modify this function...
'the tick of process' SHOULD NOT increase during context switching.
*/
void timer_handler(struct intr_frame *iframe)
{
	ticks++;
    ...
    /* if(!scheduling) */
    if((cur_process->pid!=0) && (!scheduling) && (cur_process->state==PROC_RUN))
    {

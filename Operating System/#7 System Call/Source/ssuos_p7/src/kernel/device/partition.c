...
// struct partition
//   {
//     struct blk_dev *block;                /* Underlying block device. */
//     block_sector_t start;               /* First sector within device. */
//   };

// static struct blk_dev_ops partition_operations;

// static void read_partition_table (struct blk_dev *, block_sector_t sector,
//                                   block_sector_t primary_extended_sector,
//                                   int *part_nr);
// static void found_partition (struct blk_dev *, uint8_t type,
...
// void
// partition_scan (struct blk_dev *block)
// {
//   int part_nr = 0;
//   read_partition_table (block, 0, 0, &part_nr);
//   if (part_nr == 0)
//     printk ("%s: Device contains no partitions\n", block_name (block));
// }
...
// static void
// read_partition_table (struct blk_dev *block, block_sector_t sector,
//                       block_sector_t primary_extended_sector,
//                       int *part_nr)
// {
...
//     {
//       printk ("%s: Partition table at sector %"PRDSNu" past end of device.\n",
//               block_name (block), sector);
//       return;
//     }

//   /* Read sector. */
//   ASSERT (sizeof *pt == BLK_SECTOR_SIZE);
//   pt = malloc (sizeof *pt);
//   if (pt == NULL)
...
//         printk ("%s: Invalid partition table signature\n", block_name (block));
//       else
//         printk ("%s: Invalid extended partition table in sector %"PRDSNu"\n",
//                 block_name (block), sector);
//       free (pt);
//       return;
//     }
...
//         {
//           printk ("%s: Extended partition in sector %"PRDSNu"\n",
//                   block_name (block), sector);
...
// static void
// found_partition (struct blk_dev *block, uint8_t part_type,
//                  block_sector_t start, block_sector_t size,
//                  int part_nr)
// {
...
//   else
//     {
//       enum blk_dev_type type = (part_type == 0x20 ? BLOCK_KERNEL
...
//    have room for BLK_SECTOR_SIZE bytes. */
// static void
...
//    contain BLK_SECTOR_SIZE bytes.  Returns after the block has

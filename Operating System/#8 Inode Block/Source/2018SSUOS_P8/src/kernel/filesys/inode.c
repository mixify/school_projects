...
int inode_write(struct inode *in, uint32_t offset, char * buf, int len)
{
    ...
    uint32_t blk_cnt = lbn_end - lbn_start;

    if(offset > in->sn_size)//excess size
        return -1;


    for (i = lbn_start; i <= lbn_end; ++i) {
        pbn = lbn_to_pbn(in, i);
        //should change way to lbn start end...
        memset(tmpblock, 0, SSU_BLOCK_SIZE);//init
        ...
        {
            if(i == lbn_end)
                memcpy(tmpblock + res_off, buf, len);//put datas to buf
            else
                memcpy(tmpblock + res_off, buf, SSU_BLOCK_SIZE - res_off);//put datas to buf
        }
        else if(i == lbn_end)
            memcpy(tmpblock, buf + (i-lbn_start)*SSU_BLOCK_SIZE, (offset+len) % SSU_BLOCK_SIZE);//put datas to buf
        else
            memcpy(tmpblock ,buf + (i-lbn_start)*SSU_BLOCK_SIZE, SSU_BLOCK_SIZE);//put datas to buf
        fs_writeblock(fs, pbn, tmpblock);//go to block and write
    }

    if(in->sn_size < offset+len)
        in->sn_size = offset+len;
    sync_inode(fs, in);//where should i put it.. be careful..
...
int inode_read(struct inode * in, uint32_t offset, char * buf, int len)
{
    ...
    for (i = lbn_start; i <= lbn_end; ++i) {
        ...
        {
            if(i == lbn_end)
                memcpy(buf, tmpblock + res_off, len);//put datas to buf
            else
                memcpy(buf, tmpblock + res_off, SSU_BLOCK_SIZE - res_off);//put datas to buf
        }
        else if(i == lbn_end)
            memcpy(buf - res_off + (i-lbn_start) * SSU_BLOCK_SIZE, tmpblock, (offset+len)%SSU_BLOCK_SIZE);//put datas to buf
        else
            memcpy(buf - res_off + (i-lbn_start) * SSU_BLOCK_SIZE, tmpblock, SSU_BLOCK_SIZE);//put datas to buf
        fs_writeblock(fs, pbn, tmpblock);//go to block and write
    }
...
int lbn_to_pbn(struct inode * in, uint32_t lbn )
{
    ...
    if(lbn <= 1)//direct blocks also need to be get allocated here
    {
        if(lbn >= in->sn_nlink)//res_off = 0 means what... it fits block size or what??... think later
        {
            balloc(fs->fs_blkmap, &(in->sn_directblock[lbn]));//allocate block index
            in->sn_nlink++;
            sync_bitmapblock(fs);
        }
    ...
    {
        uint32_t indirect_off = (lbn-2) / 1024;//1024 is each indirect block's max number of block
        uint32_t res_off = (lbn-2) % 1024;//1024 is each indirect block's max number of block

        memset(tmpblock_indirect, 0, SSU_BLOCK_SIZE);
        if(in->cnt_data_block == 0)
            for (int i = 0; i < NUM_INDIRECT; ++i)
                in->sn_indirectblock[i] = 0;

        if(in->sn_indirectblock[indirect_off] == 0)//no indirect block
        {
            balloc(fs->fs_blkmap, &(in->sn_indirectblock[indirect_off]));//allocate block index
            in->sn_nlink++;
            sync_bitmapblock(fs);
        }
        else
            fs_readblock(fs, in->sn_indirectblock[indirect_off], tmpblock_indirect);

        pbn = *((uint32_t *)(tmpblock_indirect + res_off*4));

        if(pbn == 0)//no data block in indirect block
        {
            pbn = balloc(fs->fs_blkmap, (uint32_t *)(tmpblock_indirect + res_off*4));//allocate block index
            in->sn_nlink++;
            in->cnt_data_block++;					// count of indirect  block
            fs_writeblock(fs, in->sn_indirectblock[indirect_off], tmpblock_indirect);//write allocated block index to indirect block
            sync_bitmapblock(fs);
        }
    }

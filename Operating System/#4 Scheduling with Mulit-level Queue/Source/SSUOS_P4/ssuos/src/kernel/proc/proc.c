...
void proc_wake(void)
{
    ...
    while(!list_empty(&slist))
    {
        ...
        /* printk("on wake... : %d process => t = %d , time_sleep = %d\n",p->pid, t, p->time_sleep); */
        if(p->time_sleep > t)
            break;
...
void proc_sleep(unsigned ticks)
{
    ...
    intr_enable();
    /* cur_process->time_sleep =  ticks + total; */
    cur_process->time_slice = 0;
    struct list_elem *e;

    /* printk("ticks = %d, cur_ticks = %d\n", ticks, cur_ticks); */
    /* printk("on sleep... : %d process => time_sleep = %d\n",cur_process->pid, cur_process->time_sleep); */
    /* printk("sleeping... %d ticks\n", cur_process->time_sleep); */
    /* printk("%d is at %d which gonna be deleted\n", cur_process->pid, cur_process->que_level); */

	...
	*(--top) = 6; //edi
    ...
    if(opt->foreground == TRUE)
    {
        if(list_empty(&f_list))
            p->console = cur_console;
        else
            p->console = get_console();//assign new output
        //should handle exisiting console and new console independently
        /* cur_process -> console = cur_console; */
        //how to make console write to this console??
        p->kbd_buffer = get_kbd_buffer();//assign new input
        cur_console = p->console;//now this printed on screen
        //issue : console before this printed too...
        //how should i make previous console don't point to cur_console??
        //is there ways to make each process point each cur_console
